...
These elements are described in different folders. All the modelling happens within the `openfisca_openfisca_france` folder.
...
```sh
pip install openfisca-openfisca_france
```
...
```sh
git clone git://github.com/guillaumeFerard/Openfisca-france.git.git
...
```sh
openfisca serve --port 5000 --country-package openfisca_openfisca_france
```
...
curl -X POST -H "Content-Type: application/json" \
  -d @./openfisca_openfisca_france/situation_examples/couple.json \
